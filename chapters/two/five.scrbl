                 ...
                 (eval:alts
                  #,(let ([f (lambda (datum offset col)
                      ...
                      (f (append (list (f 'define -1 1)
                                       (f '(install-polynomial-package) -1 5)
...
@(append-to-poly
  (put '=zero? '(polynomial)
       (lambda (p)
         (empty-termlist? (terms p)))))

And there aren't any convenient constructors for term lists:

@(append-to-poly
  (define (make-polynomial* variable . terms)
    (define (loop built coeff order . remaining)
      (let ([built (adjoin-term (make-term coeff order)
                                terms)])
        (if (null? remaining)
            built
